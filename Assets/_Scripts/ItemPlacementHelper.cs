...
public class ItemPlacementHelper
{
    ...
    public ItemPlacementHelper(HashSet<Vector2Int> roomFloor, HashSet<Vector2Int> roomFloorNoCorridor){
        ...
        foreach (var position in roomFloorNoCorridor){
            ...
            int neighborCount8Dir = graph.GetNeighbors8Directions(position).Count;
            PlacementType type = neighborCount8Dir < 8 ? PlacementType.NearWall : PlacementType.OpenSpace;
    ...
    // This handles the case where no position was found in the given iterations
    return null;
}
